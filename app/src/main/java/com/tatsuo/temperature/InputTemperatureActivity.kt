...
class InputTemperatureActivity : AppCompatActivity(), TimePickerFragment.OnTimeSelectedListener, DatePickerFragment.OnDateSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(temperatureId != -999L) {
            val dao = TemperatureListFragment.database.temperatureDao()
            val handler = Handler()
    ...
    private fun getTemperatureData() {
        ...
        myExecutor.execute() {
            ...
            var temperatureList: List<Temperature> = dao.getAllTemperatureData()

            Log.e("***Temperature***", "getTemperatureData()")

            for (temperature in temperatureList) {
                Log.e("***Temperature***", "" + temperature.id + " : " + temperature.personId
                        + " : " + temperature.date + " : " + temperature.temperature + " : " + temperature.useAntipyretic
                        + " : " + temperature.conditions + " : " + temperature.uuid + " : " + temperature.memo)
            }
        }
